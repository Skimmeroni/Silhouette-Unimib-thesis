...
\begin{document}
	...
	\thispagestyle{empty}
		...
		Nel presente lavoro ne verrà presentata una in particolare, Silhouette.
		Verrà introdotta tale metrica, come calcolarla, alcune proprietà
		matematiche e verrà applicata al risultato del clustering su delle
		...
		considerazioni in merito alla metrica stessa e alla sua capacità
	...
	\setcounter{page}{1}
		...
		È però comunque possibile osservare l'operato di un algoritmo
		di clustering ``a posteriori'' e studiarlo matematicamente.

		Questo approccio viene seguito da degli \textbf{indici} (talvolta
		chiamati \textbf{metriche} o \textbf{coefficienti}) formulati
		specificatamente per questo scopo. Così come la media o la
		deviazione standard riassumono in un solo numero una proprietà
		di un insieme di dati (il suo centro nel primo caso e la sua
		dispersione nel secondo), indici di questo tipo riassumono in
		un solo numero la bontà dell'operato di un algoritmo di clustering.
		...
		Fra queste, per questa tesi è stata scelta Silhouette
		...
		(EHR). Questo è routine nella letteratura scientifica: alcuni
		articoli che sono andati in questa direzione sono riassunti
		nella Tabella~\ref{tbl:ehr}. La Tabella~\ref{tbl:theory} riassume
		invece alcuni articoli che analizzano le proprietà di Silhouette
		e ne propongono delle varianti, e che verranno citati nelle
	...
	\chapter{Metodi e strumenti utilizzati}
		...
		\section{Algoritmi di clustering}
			...
			\subsection{Clustering per densità: DBSCAN e HDBSCAN}
				...
				in automatico, e determina quali cluster figurano più spesso
				in ciascuna iterazione. Per tal motivo, la scelta di MinPts
				per HDBSCAN è stata analoga a quella per DBSCAN, ovvero
		...
		\section{Calcolo di Silhouette}
			...
			rispettivamente con $a(i)$ e $b(i)$. A partire da queste sarà
			possibile calcolare un valore $s(i)$, e la media di tutti gli
			$s(i)$ per ciascun $i$ sarà il valore di interesse.
			...
			proporzionale a $s(i)$. Un esempio è riportato in Figura~\ref{fig:si}.
			...
			\begin{figure}[H]
				...
				$i$ è riportato il valore di $s(i)$ ed il cluster a cui $i$
				è stato assegnato.}
				\label{fig:si}
			\end{figure}
			...
			elemento dell'intero dataset. Più il valore della Silhouette
			media complessiva si avvicina ad $1$, più il clustering
			...
			conseguenza, la Silhouette media complessiva sarà bassa.
			...
			\begin{figure}[H]
				...
				dataset costituito da punti di normali bivariate, dove
				il numero di cluster è inferiore del numero di cluster
			...
			\begin{figure}[H]
				...
				dataset costituito da punti di normali bivariate, dove
				il numero di cluster è superiore al numero di cluster
				naturali. Si noti il valore della Silhouette media
				complessiva basso.}
				\label{fig:k6}
			\end{figure}
		...
		\section{Test sanity check e matrice binaria}
			...
			avere la certezza di reperire solamente pacchetti di qualità.
			...
			considerata una implementazione fidata, è possibile usarla come
			...
			è che una implementazione corretta di Silhouette fornisca un
			...
			introdotti valori casuali, si abbassino sempre più. Pur
			ammettendo delle fluttuazioni, ci si aspetta che tali
	...
	\chapter{Risultati ottenuti}
		...
		\section{Risultati delle applicazioni di algoritmi di clustering su EHR}
			...
			clustering è sempre quello che ha indotto il clustering che
	...
	\chapter{Discussione}
		...
		\section{Classificazione dei pacchetti R}
			...
			Sulla base dei test sanity check e matrice binaria è possibile
			trarre delle conclusioni in merito a quale pacchetto R che
			implementa Silhouette è da preferire.
			...
			del test era di creare una situazione così estrema da evidenziare
			...
			è contenuta, e decresce con l'aumentare dell'aumentare delle
		...
		\section{Performance degli algoritmi di clustering su EHR}
			...
			tratta di parametri massimizzati sulla base della Silhouette
			media complessiva. Questo è effettivamente in accordo con
			gli articoli citati
			\cite{doi:10.3233/IDA-2012-0545}.
			...
			questo possa sembrare anomalo, è invece un risultato a
			favore delle capacità di Silhouette.
			...
			questo significa che il dataset è effettivamente privo di
			...
			algoritmo che hanno restituito i più alti valori della Silhouette
			...
			con una certa combinazione di parametri, è riuscito a raggiungere.
			...
			Figura~\ref{fig:dbscan5}, ma dove il clustering è stato fatto
			usando come valore di MinPts $D + 1$ e come valore di $\epsilon$
			il valore ricavato osservando il KNN plot del dataset. Come è
		...
		\section{Oltre Silhouette?}
			...
			Le idee alla base di Silhouette possono essere estese in
			innumerevoli modi, alcuni dei quali suggeriti dagli autori
			stessi. Ad esempio, sarebbe possibile calcolare la Silhouette
			media complessiva non calcolando il valore di Silhouette per
			ciascun elemento del dataset e poi facendo la media rispetto
			alla cardinalità dell'intero dataset, bensì calcolando la
			Silhouette media per ciascun cluster e poi facendo la media rispetto
			al numero di cluster.

			Questo approccio non si rivela però fruttifero. Sono state infatti
			testate \cite{10.1007/978-3-319-19369-4_5} entrambe le versioni
			della Silhouette media complessiva sia su dataset artificiali,
			sia su dataset reali come \texttt{iris}, per vedere se fra le
			due si presentano differenze. In effetti, il metodo di calcolo
			originale riesce ad individuare il numero corretto di cluster
			in sostanzialmente tutti i casi, mentre il secondo tende a
			fallire (talvolta in maniera anche grossolana).

			Un'altra proposta degli autori è quella di usare la Silhouette
			direttamente nella funzione obiettivo da massimizzare all'interno
			di un algoritmo di clustering, anziché usarla in una valutazione
			a posteriori (in sostanza, basare il criterio di terminazione
			dell'algoritmo sulla Silhouette media complessiva).

			Anche in questo caso, però, l'approccio non si rivela
			fruttifero. Sono state infatti testate \cite{10.1007/978-3-030-52348-0_2}
			le prestazioni di un algoritmo che effettivamente usa la Silhouette
			nella sua funzione obiettivo, ovvero \textbf{Recursive Partitioning
			Clustering Tree} (\textbf{RPCT}). In tutti i dataset utilizzati
			(sia creati ad hoc, sia dataset reali), le prestazioni di RPCT
			si sono però rivelate scadenti. L'autore evidenzia come la
			Silhouette media complessiva possa essere utilizzata come
			criterio di terminazione con prestazioni accettabili solamente 
			se la forma del dataset è "tipica", come una distribuzione
			gaussiana.

			Un'ultima proposta avanzata dagli autori riguarda una possibile
			``scorciatoia''. Infatti, se l'algoritmo di clustering sul cui
			operato viene applicata Silhouette si basa sulla costruzione di
			centroidi o sulla elezione di rappresentanti (come K-Means), si
			potrebbe usare la distanza da tali centroidi o rappresentanti
			come grado di dissomiglianza anziché calcolare $a(i)$ o $D(i, C)$
			per ogni $i$-esimo elemento, semplificando il procedimento.
			Naturalmente, questo avrebbe lo svantaggio di ridurre la
			portabilitá di Silhouette, non potendo piú venire applicata a 
			qualsiasi algoritmo.

			Questo approccio é stato effettivamente tentato con successo
			\cite{NAGHIZADEH2020205}. Gli autori dello studio hanno modificato
			la formula del calcolo di $a(i)$ e $b(i)$ di modo che questa
			considerasse solamente i centroidi dei cluster e non tutti i
			punti, ed hanno testato le capacità di tale formula sul risultato
			di K-Means su diversi dataset. Ciò che è emerso è che le capacità
			predittive di questa nuova Silhouette, chiamata \emph{Condensed
			Silhouette}, sono leggermente inferiori a quelle della Silhouette
			originale, ma è quasi il doppio più veloce da calcolare.

			Infine, Silhouette è stata in origine pensata perché operasse su
			valori continui, ma articoli successivi hanno tentato di estendere
			Silhouette a valori discreti. In particolare, la metrica
			\textbf{CUBOS} \cite{Gao2019CUBOSAI} combina l'approccio usato da
			Silhouette (che gli autori giudicano il migliore) con una funzione
			di distanza appositamente pensata per dati categoriali, e le
			performance di tale metrica sono promettenti.

	\chapter{Conclusioni}

		Data la natura di questa tesi, è evidente come questo non possa
		...
		sono una risorsa onnicomprensiva. Si potrebbe pertanto ripetere i
		test utilizzando anche o soltanto dataset di dati biomedici che
		contengono dati che le EHR ignorano, dando un quadro più completo.
	...
	\chapter*{Disponibilità del codice}
		...
		Il codice in questione può essere reperito al seguente link:
